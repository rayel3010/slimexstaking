...
function App() {
  ...
  function handleStakeDemo() {
    ...
    if (amountToStake <= 0) return alert("Enter valid amount.");

    setStakedCount(stakedCount + amountToStake);
    setRewardPoints(rewardPoints + amountToStake * 10);
    setAmountToStake(0);
  }
    ...
    <div className="min-h-screen bg-night bg-radial-grid text-white flex">
      ...
      <aside className="w-64 border-r border-white/5 bg-[#050816]/80 backdrop-blur-xl flex flex-col">
        <div className="flex items-center gap-3 px-6 py-6 border-b border-white/5">
          <img src="/1111.png" className="w-10 h-10 rounded-2xl shadow-neon object-cover" />
          <div className="leading-tight">
            <div className="font-semibold text-sm tracking-wide">SlimeX Staking</div>
            <div className="text-[11px] text-text-dim uppercase tracking-widest">Reward Points</div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col">
        <header className="flex items-center justify-between px-8 py-5 border-b border-white/5 backdrop-blur-xl bg-night/60">
          <div>
            <div className="text-xs uppercase tracking-[0.25em] text-text-dim mb-1">SlimeX Staking Dashboard</div>
    ...
    <div className="flex-1 p-8 grid grid-cols-1 xl:grid-cols-[2fr,1.3fr] gap-8">
      ...
      <section className="space-y-6">
        <div className="rounded-3xl border border-white/10 bg-night-soft/80 backdrop-blur-xl p-7 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-text-dim mb-2">SlimeX Points Program</p>
              <h2 className="text-xl md:text-2xl font-semibold mb-3">Stake SlimeX NFTs, earn reward points.</h2>
              <p className="text-sm text-text-dim max-w-md">
                Each staked NFT generates <span className="text-neon-cyan font-medium">SlimeX Points</span> per day.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 text-sm min-w-[170px]">
              <div className="rounded-2xl border border-neon-cyan/40 bg-white/5 px-4 py-3">
                <div className="text-[11px] text-text-dim uppercase mb-1">Your Staked NFTs</div>
                <div className="text-xl font-semibold">{isConnected ? stakedCount : "-"}</div>
              </div>

              <div className="rounded-2xl border border-neon-pink/40 bg-white/5 px-4 py-3">
                <div className="text-[11px] text-text-dim uppercase mb-1">Reward Points</div>
                <div className="text-xl font-semibold">
                  {isConnected ? rewardPoints.toLocaleString() : "-"}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-night-soft/80 backdrop-blur-xl p-6 space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-text-dim">Enter the number of NFTs</p>
            </div>
            <div className="hidden md:block text-[11px] text-text-dim">
              Reward rate: <span className="text-neon-cyan font-medium">10 pts / NFT / day</span>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={onStake}
                className="px-5 py-2 rounded-xl bg-gradient-to-r from-neon-cyan to-neon-purple text-black font-semibold text-sm shadow-neon-soft hover:opacity-90 transition"
              >
                Stake Selected
              </button>
              ...
              <button
                onClick={onUnstake}
                className="px-5 py-2 rounded-xl text-sm border border-white/15 text-text-dim hover:border-neon-cyan/60 hover:text-neon-cyan transition"
              >
                Unstake All
              </button>
            </div>
      ...
      <section className="space-y-6">
        <div className="rounded-2xl border border-white/10 bg-night-soft/80 backdrop-blur-xl p-6">
          ...
          <ul className="text-xs text-text-dim list-disc list-inside space-y-2">
            <li>Each staked NFT generates 10 points/day</li>
            <li>Points accumulate while staked</li>
            <li>Unstaking stops accumulation</li>
            <li>Redeem rules vary by campaign</li>
          </ul>
        </div>
      </section>
    </div>
    ...
    <div className="flex-1 p-8 grid grid-cols-1 xl:grid-cols-2 gap-8">
      ...
      <section className="space-y-6">
        ...
        <div className="rounded-2xl border border-white/10 bg-night-soft/80 backdrop-blur-xl p-6">
          ...
          <p className="text-xs text-text-dim mb-3">Select the primary network used by SlimeX Staking.</p>
        ...
        {/* POINTS HISTORY */}
        <div className="rounded-2xl border border-white/10 bg-night-soft/80 backdrop-blur-xl p-6">
          <h3 className="font-semibold text-sm mb-3">Points History (Preview)</h3>
          <p className="text-xs text-text-dim mb-3">Demonstration table for user point activity.</p>
      ...
      <section>
        <div className="rounded-2xl border border-neon-purple/40 bg-gradient-to-br from-neon-purple/15 to-night-soft/80 p-6 shadow-neon-soft">
          ...
          <p className="text-xs text-text-dim mb-4">Official links for the SlimeX ecosystem.</p>
